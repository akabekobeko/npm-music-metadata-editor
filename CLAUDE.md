...
- **`class` と `interface` を使わない**。`function` と Plain Object を組み合わせて実装する。
- **共通の型は `type` で定義**する。`interface` は使わない。
...
- `type` は **責務単位で 1 ファイル 1 型**を基本とする。複数の型をまとめる場合は意味的に強い結びつきがあるものに限る。
- 列挙的な値は **as const + Union 型**で表現する (`enum` は使わない)。
...
src/
  ...
  types/                # 共通の type 定義 (TagData, AudioFormat, PictureInfo, ...)
  utils/                # 汎用ユーティリティ
docs/
  README.md             # 実装計画の目次
