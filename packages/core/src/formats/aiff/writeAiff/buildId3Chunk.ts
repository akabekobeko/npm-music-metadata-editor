...
import { AIFF_CHUNK_ID3 } from "../constants.js";
...
/**
 * Build the AIFF `ID3 ` chunk to emit, or return an empty buffer when
 * nothing needs to be written.
 *
 * The chunk is emitted whenever the source already carried one (so a
 * round-trip does not lose the tag), whenever the caller supplied any
 * recognised tag field, or whenever any extras (pictures / chapters /
 * lyrics) were supplied. The ID3v2 major version mirrors the source's
 ...
 * CHAP, …) and not in the user-supplied extras pass through verbatim.
 *
