 ...
 *
 * `fileName` is always prepended when the caller's `visibleIds` omits it:
 * the column carries the row identity, so the grid is unusable without it.
 * Duplicate ids are de-duplicated while preserving the caller's order.
 *
 * The format support matrix is accepted for signature stability — column
 * shape itself never depends on `support`. Per-cell write gating goes through
 * `isCellWritable`.
 *
