...
describe("writeId3v1", () => {
  ...
  it("truncates strings that exceed the field length", () => {
    ...
    // Title field is 30 bytes starting at offset 3.
    expect(Array.from(bytes.subarray(3, 33))).toEqual(Array.from({ length: 30 }, () => 0x41));
    // The byte right after must remain zero (no overflow into the artist field).
  ...
  it("resolves a known genre name when no code is provided", () => {
    const bytes = writeId3v1({
      ...
      year: "",
      comment: "",
      genre: "Rock", // genre index 17
      genreCode: -1, // invalid → fall back to lookup
    });
