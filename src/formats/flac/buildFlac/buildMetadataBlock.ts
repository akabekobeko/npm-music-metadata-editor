 ...
 */
export const buildMetadataBlock = ({ type, data, isLast }: Args): Uint8Array => {
  if (data.length > 0xffffff) {
    throw new RangeError(`buildMetadataBlock: body length ${data.length} exceeds 24-bit max`);
  }

  const out = Buffer.alloc(FLAC_METADATA_BLOCK_HEADER_SIZE + data.length);
  const typeByte = (type & FLAC_BLOCK_TYPE_MASK) | (isLast ? FLAC_LAST_BLOCK_FLAG : 0);
  out.writeUInt8(typeByte, 0);
  out.writeUIntBE(data.length, 1, 3);
  out.set(data, FLAC_METADATA_BLOCK_HEADER_SIZE);
